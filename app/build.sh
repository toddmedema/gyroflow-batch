...
if [[ -f "${REPO_ROOT}/icon.png" ]]; then
    ...
    ICON_PYTHON="${ICON_PYTHON:-/usr/bin/python3}"
    "${ICON_PYTHON}" "${SCRIPT_DIR}/normalize_icon.py" "${REPO_ROOT}/icon.png" "${NORM}"
...
cp "${REPO_ROOT}/gyroflow_batch_helpers.py" "${CONTENTS}/Resources/gyroflow_batch_helpers.py"
cp "${REPO_ROOT}/video_extensions.txt" "${CONTENTS}/Resources/video_extensions.txt"
cp "${REPO_ROOT}/resolve_gyroflow_timeline.py" "${CONTENTS}/Resources/resolve_gyroflow_timeline.py"
